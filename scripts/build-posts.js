...
async function fetchPostsFromAPI() {
  try {
    ...
    if (response.statusCode !== 200) {
      throw new Error(`API request failed with status ${response.statusCode}. Is the API server running?`);
    }
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      console.error('   or');
      console.error('   cd functions/aws && export $(cat .env | grep -v "^#" | xargs) && npm run dev');
      console.error('');
    }
...
function createBuildManifest(versionTag, posts) {
  ...
  const versionedManifest = path.join(BACKUP_DIR, `${versionTag}.build-manifest.json`);
  fs.writeFileSync(versionedManifest, JSON.stringify(manifest, null, 2), 'utf8');
...
function cleanOldBackups() {
  try {
    ...
    filesToDelete.forEach(file => {
      ...
      const manifestFile = file.path.replace('.posts.ts', '.build-manifest.json');
...
async function buildPosts() {
  try {
    ...
    console.log(`📊 Generated ${posts.length} posts with version ${versionTag}`);
